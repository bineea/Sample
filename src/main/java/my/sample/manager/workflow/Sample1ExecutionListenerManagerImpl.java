...
@Component
public class Sample1ExecutionListenerManagerImpl extends AbstractManager implements ExecutionListener {
    ...
    public void notify(DelegateExecution delegateExecution) {
        System.out.println("============触发执行监听器1开始=============");
        System.out.println("============当前流程实例ID："+delegateExecution.getProcessInstanceId()+"============");
        System.out.println("============当前节点："+delegateExecution.getCurrentActivityId()+"============");
        List<User> userList = userRepo.findAll();
        userList.stream().forEach(u -> logger.info("用户信息--------------"+u.toJson()));
        System.out.println("============触发执行监听器1结束=============");
    }
}
